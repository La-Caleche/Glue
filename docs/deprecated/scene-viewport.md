...
`AbstractViewportScreen<C>` is a Minecraft `Screen` that renders a 3D scene into an FBO, blits it
as a fullscreen quad, and handles camera interaction (drag, scroll, FPS capture). The type
parameter is the concrete camera controller passed to the constructor — the protected
`cameraController` field (and `getCameraController()`) keep that type, so subclasses never
downcast.

```java
public class MyViewportScreen extends AbstractViewportScreen<OrbitCameraController> {
