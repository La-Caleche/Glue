...
// Get a specific entry, registering a permanent Java fallback on a miss (never null).
// The supplier only runs when neither layer has the id; a JSON entry with the same id
// still overrides the fallback, and its removal on reload falls back to it cleanly.
// The supplier must produce a value: a null supplier or a null result throws (naming the id)
// rather than leaving the id registered-but-unresolvable.
GluePipeline pipeline = GlueClientRegistries.PIPELINES.getOrRegister(myId,
        () -> GluePipeline.builder(myId, vertexShader, fragmentShader).build());
