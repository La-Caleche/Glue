...
## 6. Lumos under an active shaderpack

An active shaderpack runs Lumos in reduced mode because the pack owns its base geometry programs and
`colortex` layout. Lumos borrows the pack's scene depth, re-renders nearby terrain, living entities,
glass, water and metal into its own material attachments, performs deferred lighting, and composites
the result through the Iris post-effect seam. Surfaces not covered by those self-contained captures
use the reduced fallback. If Iris internals drift and a coherent scene depth is unavailable, Lumos
turns itself off for the frame rather than corrupting the pack output. See
[Dynamic Lights](lights.md#performance--limitations) for the complete constraints.

Iris **installed with shaders switched off** remains fully supported: Iris is dormant,
`isIrisShaderEnabled()` returns false, and Lumos runs its normal vanilla Fancy path with full material
capture.
