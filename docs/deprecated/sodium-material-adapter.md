...
pack-defined `colortex` layout — attachments 1–3 are the pack's, not ours. Lumos instead runs its
reduced path: it borrows Iris scene depth and fills its own material attachments through
self-contained terrain, entity, glass, water and metal re-renders before compositing through the
post-effect seam. The Sodium adapter therefore covers the vanilla/Sodium **Fancy** path; it is not
used while an active pack owns terrain rendering. Fabulous graphics still disables Lumos because
its separate translucent target does not match the main scene depth. See
