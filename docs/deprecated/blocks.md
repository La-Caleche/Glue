...
public static final BlockEntityType<MyBlockEntity> MY_BE = BLOCK_ENTITIES.register(
        "my_block_entity", MyBlockEntity::new, MY_BLOCK);
```

The factory receives the freshly created `BlockEntityType` as its first argument
(`(type, pos, state) -> new MyBlockEntity(type, pos, state)`), so an entity that passes its own
type to the `BlockEntity` constructor needs no self-referential supplier.

A pre-built type can still be registered directly:
