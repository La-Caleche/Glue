...
The showcase opts its integrated server into this channel. Client requests take effect only after
the server accepts them and synchronizes the resulting world-light state.
...
- **Shadow budget:** at most **15 spot/gobo** and **15 point** lights have shadow
  maps at a time; further shadow-casting lights still illuminate, just without
  shadows. The budget is configurable:

  ```java
  LightRenderer.setShadowBudget(15, 15);   // spots/gobos, points
  ```
...
- **Update budget:** resident maps and new map renders are controlled separately.
  By default Lumos renders at most five new spot maps and five new point cubemaps per
  frame, preserving already-cached maps and spreading bursts across frames:

  ```java
  LightRenderer.setShadowUpdateBudget(5, 5);
  ```
...
- **Active Iris shaderpacks** run Lumos in a **reduced mode**. The structural boundary is
  ...
  pack's framebuffer, lit against the main target, blitted back. Nearby living entities
  are also re-rendered into the material G-buffer; particles remain uncaptured on pack
  frames and use the reduced fallback. An
