...
public class MyBlockEntityRenderer implements BlockEntityRenderer<MyBlockEntity> {
    ...
    public void render(MyBlockEntity entity, float partialTick, PoseStack poseStack,
        ...
        } // close() ends the batch, then frees the vertex buffers
    }
}
```

Try-with-resources is the one blessed lifecycle: `close()` runs any pending `endBatch()` before
freeing the native vertex buffers, so nothing leaks — even when a draw call throws. An explicit
`endBatch()` mid-block is still allowed (it is a no-op when nothing was drawn since the last
flush, so `close()` never flushes twice).
...
3. On `endBatch()` (run automatically by `close()` when a batch is still pending):
