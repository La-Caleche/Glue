...
public final class GlLightRenderer {
    ...
    /** Scales the linear illumination before the final display-space composite. */
    private static final float EXPOSURE = 6.0f;
     ...
     */
    private static final float TINT_BLUR_STRIDE = 1.0f;
     ...
     * @param materialColorId linear terrain albedo and packed normal, or -1
     * @param materialDepthId depth captured with the terrain material, or -1
     */
    public void accumulateLight(int lightFboId, int sceneDepthId, Matrix4f viewProj, Matrix4f invViewProj,
                                Vector3d cameraPos, Light light, int width, int height,
                                 int[] bounds, @Nullable ShadowParams shadow,
                                 int glassColorId, int glassDepthId,
                                 int materialColorId, int materialDepthId) {
        ...
        uniform3f(program, "SpotDir", light.directionX, light.directionY, light.directionZ);
        ...
        boolean hasMaterial = materialColorId > 0 && materialDepthId > 0;
        GL13.glActiveTexture(GL13.GL_TEXTURE7);
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, hasMaterial ? materialColorId : 0);
        GL13.glActiveTexture(GL13.GL_TEXTURE8);
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, hasMaterial ? materialDepthId : 0);
        uniform1i(program, "MaterialAlbedo", 7);
        uniform1i(program, "MaterialDepth", 8);
        uniform1i(program, "HasMaterial", hasMaterial ? 1 : 0);
     ...
     *
     * @param sceneColorTexId copy of the scene colour, used as an albedo fallback
     * @param materialColorId linear terrain albedo, or -1 when unavailable
     * @param materialDepthId depth captured with the terrain albedo, or -1
     */
    public void compositeAdditive(int lightColorTexId, int sceneColorTexId, int sceneDepthId,
                                  int materialColorId, int materialDepthId,
        ...
        boolean hasMaterial = materialColorId > 0 && materialDepthId > 0 && sceneDepthId > 0;
        GL13.glActiveTexture(GL13.GL_TEXTURE2);
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, hasMaterial ? materialColorId : 0);
        GL13.glActiveTexture(GL13.GL_TEXTURE3);
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, hasMaterial ? materialDepthId : 0);
        GL13.glActiveTexture(GL13.GL_TEXTURE4);
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, hasMaterial ? sceneDepthId : 0);
        uniform1i(program, "MaterialAlbedo", 2);
        uniform1i(program, "MaterialDepth", 3);
        uniform1i(program, "SceneDepth", 4);
        uniform1i(program, "HasMaterial", hasMaterial ? 1 : 0);
    ...
    private static Matrix4f buildGoboMatrix(Light light, Vector3d cameraPos) {
        ...
        org.joml.Vector3f center = new org.joml.Vector3f(eye)
                .add(light.directionX, light.directionY, light.directionZ);
        org.joml.Vector3f up = Math.abs(light.directionY) > 0.99f
    ...
    private void setupCompositeState() {
        GL11.glColorMask(true, true, true, true);
        GL11.glDisable(GL11.GL_BLEND);
        GL11.glDisable(GL11.GL_DEPTH_TEST);
        GL11.glDepthMask(false);
