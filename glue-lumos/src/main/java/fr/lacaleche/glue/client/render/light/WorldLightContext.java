...
public final class WorldLightContext implements AutoCloseable {
    ...
    final Map<Light, List<BlockPos>> glassBlocks = new IdentityHashMap<>();

    WorldLightContext(ClientLevel level) {
        this.level = level;
    }
    ...
    synchronized void remove(LightHandle handle) {
        handles.remove(handle);
        Light resolved = handle.resolved();
        if (resolved != null) {
            glassBlocks.remove(resolved);
            shadows.remove(resolved);
        }
        handle.markRemoved();
    }

    synchronized void update(LightHandle handle, Light light) {
    ...
    synchronized void resetRenderCaches() {
