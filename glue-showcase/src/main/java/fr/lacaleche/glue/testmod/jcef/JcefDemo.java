...
import fr.lacaleche.jcef.CefRuntime;
import fr.lacaleche.jcef.CefScreen;
import fr.lacaleche.jcef.CefSurface;
import fr.lacaleche.jcef.LocalPages;
...
public final class JcefDemo {
    ...
    public static void init() {
        ...
        HudElementRegistry.addLast(TestmodClient.id("jcef"), (graphics, delta) -> {
            ...
            if (hud == null || client.screen != null || client.level == null) return;
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            CefSurface surface = client.screen instanceof CefScreen screen ? screen.surface() : hud;
            if (surface == null) return;
            surface.drainMessages(message -> {
                commands++;
    ...
    public static void openHud(Minecraft client) {
        closeHud();
        LocalPages server = pages();
        hud = new CefSurface(server.url(), true, client.getWindow().getGuiScaledWidth() * 2,
                client.getWindow().getGuiScaledHeight() * 2, server.origin());
        client.setScreen(null);
    }

    public static CefSurface hud() { return hud; }
