...
public final class ShadowPipelines {
    ...
    public static RenderType tintColor() {
        // MIPPED, and it is load-bearing: glue_shadow_tint.fsh samples
        // textureLod(.., 8.0) to get the sprite's AVERAGE colour (one flat tint per
        // pane, no texture pattern projected into the pool). With mipmap=false the
        // sampler has no mip chain, the lod argument is silently ignored, and the
        // full-detail texture goes straight into the tint map.
