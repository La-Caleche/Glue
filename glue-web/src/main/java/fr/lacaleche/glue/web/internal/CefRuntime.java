...
final class CefRuntime {
    ...
    static void bootstrap() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            for (BrowserSession session : List.copyOf(SESSIONS)) session.dispatchMessages();
        });
        ...
        try {
            ...
            builder.setAppHandler(new MavenCefAppHandlerAdapter() {
                ...
                public void stateHasChanged(CefApp.CefAppState state) {
                    ...
                    if (state == CefApp.CefAppState.TERMINATED) PROGRESS.stopped();
                }
            });
        ...
        CefMessageRouter router = CefMessageRouter.create(
                ...
                new CefMessageRouterHandlerAdapter() {
                    ...
                    public boolean onQuery(CefBrowser browser, CefFrame frame, long id, String request,
                        ...
                        if (!view.acceptsMessages(frame.getURL())) {
                            callback.failure(1, "Message origin is not enabled");
                        } else if (persistent || request.length() > BrowserSession.MAX_MESSAGE_LENGTH) {
                            callback.failure(2, "Only bounded, non-persistent messages are supported");
                        } else if (!view.messages.offer(request)) {
                            callback.failure(3, "Message queue is full");
                        } else {
                            callback.success("");
                        }
                        return true;
