...
function App() {
    ...
    useEffect(() => {
        const receive = event => setGame(JSON.parse(event.detail));
        window.addEventListener('glue:web-message', receive);
        return () => window.removeEventListener('glue:web-message', receive);
    }, []);
    window.demoSnapshot = () => ({ count, name, power, animate, effect, fps: game.fps });
    const send = message => window.glueQuery({ request: JSON.stringify(message), onSuccess() {}, onFailure(code, text) { console.error(text); } });
    return <main>
        ...
        <aside><div className="eyebrow">RENDER PATH</div><h2>BGRA → GPU</h2><p>Colors and premultiplied alpha stay intact until the fragment shader.</p><p>F3 shows delivery metrics. F9 switches the browser frame cap.</p><div className="gradient" /></aside>
    </main>;
}
