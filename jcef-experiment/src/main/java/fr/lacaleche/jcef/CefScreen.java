...
public final class CefScreen extends Screen {
    ...
    private CefSurface surface;
    private CefCursor cursor;
    ...
    public CefScreen(String url, String demoUrl, String messageOrigin, Screen previous) {
        super(Component.literal("JCEF experiment"));
    ...
    public void tick() {
        ...
        if (this.activeWindow != active) {
            this.surface.focus(active && this.getFocused() == null);
            if (!active) this.buttons = 0;
        }
        this.activeWindow = active;
        if (!active && this.cursor != null) this.cursor.reset();
    }
    ...
    public void mouseMoved(double x, double y) {
        this.pointer(this.inside(x, y) ? CefMouseEvent.MOUSEEVENT_MOVED : CefMouseEvent.MOUSEEVENT_EXITED, x, y, -1, 1);
    ...
    public boolean mouseClicked(double x, double y, int button) {
        ...
        this.pointer(CefMouseEvent.MOUSEEVENT_PRESSED, x, y, button, this.clicks);
        return true;
    }
    ...
    public boolean mouseReleased(double x, double y, int button) {
        ...
        this.buttons &= ~(1 << button);
        this.pointer(CefMouseEvent.MOUSEEVENT_RELEASED, x, y, button, this.clicks);
        this.updateCursor(x, y);
        return true;
    }
    ...
    public boolean mouseDragged(double x, double y, int button, double dx, double dy) {
        ...
        this.pointer(CefMouseEvent.MOUSEEVENT_DRAGGED, x, y, button, 1);
        this.updateCursor(x, y);
        return true;
    }
    ...
    public boolean keyPressed(int key, int scanCode, int modifiers) {
        if (key == GLFW.GLFW_KEY_ESCAPE) {
            if (this.surface.hasPopup()) this.surface.key(key, scanCode, modifiers, false);
            else this.onClose();
            return true;
        }
        ...
        if (key == GLFW.GLFW_KEY_F7) {
            this.surface.setUploadMode(this.surface.uploadMode() == CefSurface.UploadMode.GPU_BGRA
    ...
    public boolean keyReleased(int key, int scanCode, int modifiers) {
        ...
        this.surface.key(key, scanCode, modifiers, true);
        return true;
    }
    ...
    public int appliedCursorShape() { return this.cursor == null ? GLFW.GLFW_ARROW_CURSOR : this.cursor.appliedShape(); }
    ...
    private void pointer(int type, double x, double y, int button, int count) {
        this.surface.mouse(type, this.webX(x), this.webY(y), button, this.buttons, this.modifiers(), count);
    }

    private void updateCursor(double x, double y) {
        if (this.cursor == null) return;
        if (this.minecraft.screen == this && this.minecraft.isWindowActive() && this.minecraft.getOverlay() == null
                && !this.minecraft.mouseHandler.isMouseGrabbed() && this.surface != null
                && (this.inside(x, y) || this.buttons != 0)) {
            this.cursor.apply(this.surface.cursorType());
        } else {
            this.cursor.reset();
        }
    }
