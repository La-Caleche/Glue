...
final class CefView extends CefBrowserWindowless implements CefRenderHandler {
    ...
    final ArrayBlockingQueue<JsonObject> messages = new ArrayBlockingQueue<>(128);
    final AtomicReference<Probe> pendingProbe = new AtomicReference<>();
    private final AtomicReference<Boolean> requestedFocus = new AtomicReference<>();
    volatile ProbePaint probePaint;
    volatile int acknowledgedProbe;
    volatile long acknowledgedAt;
    ...
    CefView(CefClient client, String url, boolean transparent, int width, int height, long nativeWindow, String messageOrigin) {
    ...
    private CefView(CefClient client, String url, boolean transparent, int width, int height, long nativeWindow,
                    String messageOrigin, CefBrowserWindowless parent, Point inspectAt) {
    ...
    public void onPaint(CefBrowser browser, boolean isPopup, Rectangle[] rectangles, ByteBuffer buffer, int width, int height) {
        try {
            ...
            Probe probe = this.pendingProbe.get();
            if (!isPopup && probe != null && width >= 8 && height >= 8) {
                int pixel = (4 * width + 4) * 4;
                int token = Byte.toUnsignedInt(buffer.get(pixel + 2)) | Byte.toUnsignedInt(buffer.get(pixel + 1)) << 8;
                if (Byte.toUnsignedInt(buffer.get(pixel)) == 167 && token == probe.token
                        && this.pendingProbe.compareAndSet(probe, null)) {
                    this.probePaint = new ProbePaint(probe.token, probe.sentAt, System.nanoTime(), mailbox.capturedFrames());
                }
            }
        } catch (RuntimeException exception) {
    ...
    public void addOnPaintListener(Consumer<CefPaintEvent> listener) {
        throw new UnsupportedOperationException("Use the experiment's owned frame mailbox");
    }
    ...
    boolean acceptsMessages(String address) {
        if (this.messageOrigin == null) return false;
        try {
            URI uri = URI.create(address);
            return this.messageOrigin.equals(uri.getScheme() + "://" + uri.getRawAuthority());
    ...
    record Probe(int token, long sentAt) {
    }

    record ProbePaint(int token, long sentAt, long paintedAt, long sequence) {
    }
}
